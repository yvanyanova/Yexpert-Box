...
usage()
{
    ...
    Ce script permet de créer une instance de Yexpert pour GT.M
...
if grep "^$instance:" /etc/passwd > /dev/null ||
   ...
   grep "^${instance}prog:" /etc/passwd > /dev/null ; then
    # TODO: A compléter supprimerInstanceYexpertTotale.sh et supprimer les lignes ci-dessous
    su $instance -c "source $basedir/config/env && $basedir/scripts/supprimerInstanceYexpertMinimale.sh -i $instance"
fi
...
#--- perl -pi -e 's/y-instance/'$instance'/g' $basedir/config/xinetd.d/yexpert-*

# Modify init.d script to reflect $instance
perl -pi -e 's/y-instance/'$instance'/g' $basedir/config/init.d/yexpert
...
ln -s $basedir/config/init.d/yexpert /etc/init.d/${instance}yexpert

# Installer le script init
if [[ $debian || -z $RHEL ]]; then
    update-rc.d ${instance}yexpert defaults
fi

if [[ $RHEL || -z $debian ]]; then
    chkconfig --add ${instance}yexpert
fi
...
echo "L'instance d'Yexpert $instance est créée..."
