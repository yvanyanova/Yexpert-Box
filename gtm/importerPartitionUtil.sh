...
# Importer Yexpert-DMO Globals et Routines dans l'instance $instance
...
OLDIFS=$IFS
IFS=$'\n'
for routine in $(cd $basedir/src/Yexpert-${partition^^} && git ls-files -- \*.m); do
    cp $basedir/src/Yexpert-${partition^^}/${routine} $basedir/partitions/${partition,,}/routines
done
...
for global in $(cd $basedir/src/Yexpert-${partition^^} && git ls-files -- \*.zwr); do
    mupip load \"$basedir/src/Yexpert-${partition^^}/${global}\" >> $basedir/partitions/${partition,,}/log/importerGloabls.log 2>&1
done
